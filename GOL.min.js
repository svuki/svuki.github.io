...
function chunk(a,b){for(var c=[],d=0;d+b<=a.length;)c.push(a.slice(d,d+b)),d+=b;d!=a.length&&c.push(a.slice(d,a.length));return c}function flatten(a){return a.reduce(function(a,c){return a.concat(c)},[])}function pad(a,b,c){b=Array.from({length:c-a.length}).fill(b);return a.concat(b)}function flatmap(a,b){return a.reduce(function(a,d){return a.concat(b(d))},[])}function mapmap(a,b){return a.map(function(a,d){return a.map(function(c,g){return b(c,d,g,a)})})}
function arr_sum(a){return a.reduce(function(a,c){return a+c})}function range(a,b){return b?Array.from({length:b-a},function(b,d){return d+a}):Array.from({length:a},function(a,b){return b})}
function TDArray(a,b,c,d){b=void 0===b?a:b;d=void 0===d?!1:d;var f=[];for(i=0;i<a;i++)f.push(Array(b).fill(0));c&&(f=mapmap(f,function(a,b,d){return c(b,d)}));var g=d?function(b){return(b+a)%a}:function(a){return a},h=d?function(a){return(a+b)%b}:function(a){return a};this.wrap=d;this.rows=a;this.cols=b;this.show=function(){return console.log(f)};this.at=function(a,b){return d?f[g(a)][h(b)]:0>a||a>=this.rows||0>b||b>=this.cols?0:f[a][b]};this.set=function(a,b,d){return f[g(a)][h(b)]=d};this.sum=function(){return f.reduce(function(a,
...
c.handle}}function i_to_y(a,b,c){return function(d){return(d-a)*c+b}}function j_to_x(a,b,c){return function(d){return(d-a)*c+b}}function y_to_i(a,b,c){return function(d){return Math.floor((d-b)/c+a)}}function x_to_j(a,b,c){return function(d){return Math.floor((d-b)/c+a)}}
...
function Coordinate_handler(a,b,c){this.x_j=this.y_i=this.j_x=this.i_y=this.cell_size=this.rectangle=void 0;this.resize=function(d){this.cell_size=d;d=this.rectangle=calculate_rectangle(a,b,this.cell_size);this.i_y=i_to_y(d.i_0,d.y_offset,this.cell_size);this.j_x=j_to_x(d.j_0,d.x_offset,this.cell_size);this.y_i=y_to_i(d.i_0,d.y_offset,this.cell_size);this.x_j=x_to_j(d.j_0,d.x_offset,this.cell_size)};this.resize(c)}var colors={0:"white",1:"black"};
...
", rule = ",b.string].join(""),d=a.row_map(l).join("");d=d.replace(/\$$/,"!");d=d.replace(new RegExp(/\$+/,"g"),function(a){return String(a.length)+"$"});return[c].concat(chunk(d,70)).join("\n")}return{encode:function(b,c,d){c=void 0===c?classic_rule:c;return void 0===d||d?m(a(b.cells),c):m(b.cells,c)},decode:function(a){return h(a)}}}(),state={rate:1E3,step_on:!1,grid_on:!1,cell_size:10,pattern:newEmptyGameState(30,30),gamestate:void 0,rule:void 0,save_list:[],observers:{rate:[],step_on:[],grid_on:[],
cell_size:[],pattern:[],gamestate:[],rule:[],save_list:[]},add_observer:function(a,b){this.observers.hasOwnProperty(a)?this.observers[a].push(b):console.log("Error, attempted to add observer to state for nonexistent property: "+a)},notify:function(a){a in this.observers?this.observers[a].forEach(function(b){return b.update(a)}):console.log("Error, state attempted to notify on nonexistent property: "+a)}};function set_rate(a){state.rate=a;state.notify("rate")}
function toggle_step(){state.step_on=state.step_on?!1:!0;state.notify("step_on")}function toggle_grid(){state.grid_on=state.grid_on?!1:!0;state.notify("grid_on")}function set_cell_size(a){state.cell_size=a;state.notify("cell_size")}function set_gamestate(a){state.gamestate=a;state.notify("gamestate")}function get_gamestate(){return state.gamestate}function set_rule(a){state.rule=a;state.notify("rule")}function set_pattern(a){state.pattern=a;state.notify("pattern")}
function get_pattern(){return state.pattern}function snapshot(){return{gamestate:state.gamestate,rule:state.rule}}function add_to_save_list(a){state.save_list.push(a);state.notify("save_list")}function load_snapshot(a){set_gamestate(a.gamestate);set_rule(a.rule)}var gol_canvas_id="#gol-canvas",gol_canvas=$(gol_canvas_id).get(0);state.gamestate=GameStateFromCanvas(gol_canvas,5);state.rule=classic_rule;var main_c_set=new Canvas_set("#gol-canvas",10);
main_c_set.initialize(state.gamestate,10,get_gamestate,set_gamestate);state.add_observer("gamestate",main_c_set);state.add_observer("grid_on",main_c_set);state.add_observer("cell_size",main_c_set);$(main_c_set.over_canvas).unbind("click");$(main_c_set.over_canvas).click(function(a){a.shiftKey?main_c_set.merge(state.pattern,a.offsetX,a.offsetY):main_c_set.toggle(a.offsetX,a.offsetY)});
