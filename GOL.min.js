...
b){var c=this.coordinate_handler.y_i(b),d=this.coordinate_handler.x_j(a);c=this.get_gamestate().toggle(c,d);console.log(c);this.set_gamestate(c)};$(this.over_canvas).click(function(a){return c.toggle(a.offsetX,a.offsetY)});this.merge=function(a,b,c){c=this.coordinate_handler.y_i(c);b=this.coordinate_handler.x_j(b);this.set_gamestate(this.get_gamestate().merge(a,c,b))};this.project=function(){this.projector.project(this.get_gamestate())};this.toggle_grid=function(){this.grid.toggle()}}$("#toggle_grid_btn").click(function(){return toggle_grid()});
$("#soup_btn").click(function(){return set_gamestate(state.gamestate.soup())});$("#zoom-slider").on("input",function(){set_cell_size(this.value)});$("#reset_btn").click(function(){return set_gamestate(state.gamestate.clear())});$("#toggle_step_btn").click(function(){return toggle_step()});$("#rate_slider").on("input",function(){set_rate(this.value)});var rle_textarea=$("#rle_textarea");$("#rle_submit_btn").click(function(){var a=rle_textarea.val();a=rle.decode(a);set_pattern(a.gamestate)});
$("#rle_generate_btn").click(function(){var a=rle.encode(state.gamestate,state.rule);rle_textarea.val(a)});$("#save_btn").click(function(){add_to_save_list(snapshot())});function Game_of_life(a,b){var c=this;b=void 0===b?classic_rule:b;this.current_state=a;this.past=[];this.rule=b;this.step=function(){this.past.push(this.current_state);this.current_state=apply_rule(b,this.current_state)};this.snapshot=function(){return{state:c.current_state,rule:c.rule}}}function GameState(a){this.cells=a}
function GameStateFromCanvas(a,b){b=void 0===b?1:b;return newEmptyGameState(a.height/b,a.width/b)}function newEmptyGameState(a,b){return new GameState(new TDArray(a,b,function(){return 0},!1))}GameState.prototype.toggle=function(a,b){var c=0===this.cells.at(a,b)?1:0,d=this.cells.copy();d.set(a,b,c);return new GameState(d)};GameState.prototype.clear=function(){return new GameState(this.cells.map(function(){return 0}))};GameState.prototype.copy=function(){return new GameState(this.cells.copy())};
...
function Rule(a){var b=a.match("[B,b](.*)/[S,s](.*$)");this.b=b[1].split("").map(function(a){return parseInt(a,10)});this.s=b[2].split("").map(function(a){return parseInt(a,10)});this.string=a}var classic_rule=new Rule("B3/S23");function rule_fn(a){return function(b,c,d,f){c=f.nbhood(c,d,1,1).sum()-b;return 0===b&&a.b.includes(c)?1:1===b&&a.s.includes(c)?1:0}}function apply_rule(a,b){return b.map(rule_fn(a))}function draw_line(a,b,c,d,f){a.beginPath();a.moveTo(b,c);a.lineTo(d,f);a.stroke()}
function draw_horizontal_lines(a,b,c){var d=a.getContext("2d");range(Math.ceil(a.height/c)).forEach(function(f){return draw_line(d,0,b+f*c,a.width,b+f*c)})}function draw_vertical_lines(a,b,c){var d=a.getContext("2d");range(Math.ceil(a.width/c)).forEach(function(f){return draw_line(d,b+f*c,0,b+f*c,a.height)})}
...
function(a,b){b=void 0===b?c:b;d=a.cell_size;g=a.i_y;h=a.j_x;f=a.rectangle;this.whipe();this.project(b)}}
...
c.handle}}
var rle=function(){function a(a){var b=function(a){return 1===a},c=-1,d=-1,k=-1,f=-1;for(r_index=0;r_index<a.rows;r_index++)if(a.row(r_index).some(b)){c=r_index;break}for(r_index=a.rows-1;0<=r_index;r_index--)if(a.row(r_index).some(b)){k=r_index;break}for(c_index=0;c_index<a.cols;c_index++)if(a.col(c_index).some(b)){d=c_index;break}for(c_index=a.cols-1;0<=c_index;c_index--)if(a.col(c_index).some(b)){f=c_index;break}return a.rect(c,d,k,f)}function b(a){var b=a.match(/(\d*)([a-z])/);a={o:1,b:0}[b[2]];
...
function addToSaveList(a){$(saveListId).append(processSnapshot(a,generateSaveId()));return!0}
var state={rate:1E3,step_on:!1,grid_on:!1,cell_size:10,pattern:newEmptyGameState(30,30),gamestate:void 0,rule:void 0,save_list:[],observers:{rate:[],step_on:[],grid_on:[],cell_size:[],pattern:[],gamestate:[],rule:[],save_list:[]},add_observer:function(a,b){this.observers.hasOwnProperty(a)?this.observers[a].push(b):console.log("Error, attempted to add observer to state for nonexistent property: "+a)},notify:function(a){a in this.observers?this.observers[a].forEach(function(b){return b.update(a)}):
console.log("Error, state attempted to notify on nonexistent property: "+a)}};function set_rate(a){state.rate=a;state.notify("rate")}function toggle_step(){state.step_on=state.step_on?!1:!0;state.notify("step_on")}function toggle_grid(){state.grid_on=state.grid_on?!1:!0;state.notify("grid_on")}function set_cell_size(a){state.cell_size=a;state.notify("cell_size")}function set_gamestate(a){state.gamestate=a;state.notify("gamestate")}function get_gamestate(){return state.gamestate}
function set_rule(a){state.rule=a;state.notify("rule")}function set_pattern(a){state.pattern=a;state.notify("pattern")}function get_pattern(){return state.pattern}function snapshot(){return{gamestate:state.gamestate,rule:state.rule}}function add_to_save_list(a){state.save_list.push(a);state.notify("save_list")}function load_snapshot(a){set_gamestate(a.gamestate);set_rule(a.rule)}var gol_canvas_id="#gol-canvas",gol_canvas=$(gol_canvas_id).get(0);state.gamestate=GameStateFromCanvas(gol_canvas,5);
state.rule=classic_rule;var main_c_set=new Canvas_set("#gol-canvas",10);main_c_set.initialize(state.gamestate,10,get_gamestate,set_gamestate);state.add_observer("gamestate",main_c_set);state.add_observer("grid_on",main_c_set);state.add_observer("cell_size",main_c_set);$(main_c_set.over_canvas).unbind("click");$(main_c_set.over_canvas).click(function(a){a.shiftKey?main_c_set.merge(state.pattern,a.offsetX,a.offsetY):main_c_set.toggle(a.offsetX,a.offsetY)});
main_c_set.update=function(a){switch(a){case "gamestate":this.project();break;case "grid_on":this.toggle_grid();break;case "cell_size":this.coordinate_handler.resize(state.cell_size),this.projector.resize(this.coordinate_handler),this.grid.resize(state.cell_size)}};var stepper=new RateController(state.rate,function(){return set_gamestate(apply_rule(state.rule,state.gamestate))});state.add_observer("rate",stepper);state.add_observer("step_on",stepper);
stepper.update=function(a){switch(a){case "rate":this.set_rate(state.rate);break;case "step_on":this.toggle()}};var pattern_canvas_id="#pattern_canvas",pattern_c_set=new Canvas_set(pattern_canvas_id,10);pattern_c_set.initialize(state.pattern,10,get_pattern,set_pattern);pattern_c_set.toggle_grid();state.add_observer("pattern",pattern_c_set);pattern_c_set.update=function(a){switch(a){case "pattern":a=auto_cell_size(pattern_c_set.under_canvas,state.pattern),this.change_gamestate(get_pattern(),a),this.grid.resize(a)}};
$("#rle_textarea").val(patterns.backrake_1);set_pattern(rle.decode(patterns.backrake_1).gamestate);var save_list={update:function(a){switch(a){case "save_list":addToSaveList(state.save_list[state.save_list.length-1])}}};state.add_observer("save_list",save_list);
...
TDArray.prototype.merge=function(a,b,c){var d=this;b=void 0===b?0:b;c=void 0===c?0:c;b=range(b,b+a.rows);var f=range(c,c+a.cols);b.forEach(function(b,c){return f.forEach(function(f,g){return d.set(b,f,a.at(c,g))})})};TDArray.prototype.transpose=function(){var a=this;return new TDArray(this.cols,this.rows,function(b,c){return a.at(c,b)})};function chunk(a,b){for(var c=[],d=0;d+b<=a.length;)c.push(a.slice(d,d+b)),d+=b;d!=a.length&&c.push(a.slice(d,a.length));return c}
